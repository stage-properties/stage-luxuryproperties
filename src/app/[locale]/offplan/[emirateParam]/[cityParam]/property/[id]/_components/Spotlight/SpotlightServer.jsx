...
const SpotlightServer = ({
  ...
  const alternative_text =
    offplanData?.attributes?.featured_image?.data?.attributes
      ?.alternativeText || `${project_name} by ${developer_name}`;

  const aed_to_usd_exchange_rate =
    offplanData?.configuration?.data?.attributes?.aed_to_usd_exchange_rate ||
    0.27;
  const aed_to_eur_exchange_rate =
    offplanData?.configuration?.data?.attributes?.aed_to_eur_exchange_rate ||
    0.25;
  const aed_to_gbp_exchange_rate =
    offplanData?.configuration?.data?.attributes?.aed_to_gbp_exchange_rate ||
    0.25;
  const aed_to_inr_exchange_rate =
    offplanData?.configuration?.data?.attributes?.aed_to_inr_exchange_rate ||
    23.53;
  const aed_to_rub_exchange_rate =
    offplanData?.configuration?.data?.attributes?.aed_to_rub_exchange_rate ||
    24.91;
  ...
  const unit_area = offplanData?.attributes?.area_starting_from
    ? Math.round(
        Number(offplanData?.attributes?.area_starting_from) *
          (areaUnit === "ft²" ? 1 : square_feet_to_square_meter)
      )
    : t("ask_for_area");
  ...
  const calcHandoverInfoText = () => {
    const { ready_to_move_in, handover_info, handover_quarter } =
      offplanData?.attributes || {};
    let s = "";
                ...
                <div className="right">
                  {!offplanData?.attributes?.starting_price ? (
                    ...
                    <span className="value">
                      {price} {t(currency.toLowerCase())}
                    </span>
                  ) : (
                    <span className="value">
                      {t(currency.toLowerCase())} {price}
                    </span>
                  )}
