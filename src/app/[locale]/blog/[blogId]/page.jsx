...
export const generateMetadata = async ({ params }) => {
  ...
  let titleTranslation = blogInfo?.attributes?.seo?.metaTitle ? blogInfo?.attributes?.seo?.metaTitle  + "| Stage Properties" : blogInfo?.attributes?.project_name  + "| Stage Properties"
  let descriptionTranslation = blogInfo?.attributes?.seo?.metaDescription ? blogInfo?.attributes?.seo?.metaDescription : `Buy Property in Dubai - Stage Properties, Dubai's best investment property broker`
  let openGraphURL = blogInfo?.attributes?.featured_image?.data?.attributes?.url
  let openGraphAlt = blogInfo?.attributes?.featured_image?.data?.attributes?.alternativeText || titleTranslation
  let twitterImage = blogInfo?.attributes?.featured_image?.data?.attributes?.url ? blogInfo?.attributes?.featured_image?.data?.attributes?.url : "url/image.webp"
  let facebookImage = blogInfo?.attributes?.featured_image?.data?.attributes?.url ? blogInfo?.attributes?.featured_image?.data?.attributes?.url : "url/image.webp"
  let keywords = blogInfo?.attributes?.seo?.keywords ? blogInfo?.attributes?.seo?.keywords : blogInfo?.attributes?.seo?.metaTitle
  
  const translation = await getTranslation({text: {titleTranslation, descriptionTranslation}, locale})
...
const page = async ({params}) => {
  ...
  const blogInfoTranslation = await getTranslation({text: blogInfo?.attributes?.blog_title.toString().trim().toUpperCase(), locale})
  const blogCategory = blogInfo?.attributes?.blog_categories?.data[0]?.attributes?.slug

  const breadcrumbItems = [
    {
      title: <Link href='/blogs' className='breadcrumb'>{t_blogs('blogs')}</Link>
    },
    {
  ...
  `{
    ...
    "itemListElement": [{
      ...
      "position": 1,
      "name": "Home",
      "item": "https://stageproperties.com${locale === 'en' ? '' : `/${locale}`}"
    },{
      "@type": "ListItem",
      "position": 2,
      "name": "Blogs",
      "item": "https://stageproperties.com/${locale === 'en' ? '' : `${locale}/`}blogs"
    },{
      "@type": "ListItem",
      "position": 3,
      "name": "${blogInfo?.attributes?.blog_title.toString().trim()}",
      "item": "https://stageproperties.com/${locale === 'en' ? '' : `${locale}/`}blog/${slug}"
    }
    ]
  }`
    ...
    <>
      ...
      <div id="blogInfo" className='wrapper'>
          ...
          <div style={{display: 'flex', position: 'relative', justifyContent: 'space-between'}}>
            <BlogContent blogContent={blogInfo?.attributes?.content} locale={locale} blogCategory={blogCategory} blogSlug={slug} />
            <div style={{position: 'sticky', top: '105px', alignSelf: 'flex-start', height: 'calc(100vh - 100px)'}}>
