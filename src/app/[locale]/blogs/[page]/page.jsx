...
export async function generateMetadata({params}){
  ...
  if(locale !== 'en'){
    const translation = await getTranslation({text: {metaTitleTranslation, metaDescriptionTranslation}, locale})
  ...
  return {
    ...
    alternates: {
      canonical: `https://stageproperties.com/${locale === 'ar' ? 'ar/' : ''}blogs`,
      languages: {
        'en-gb': fullURL,
        'en': fullURL,
        'x-default': fullURL,
...
const page = async ({params}) => {
  ...
  if(locale !== 'en'){
    const translation = await getTranslation({text: {pageHeadingTranslation, briefIntroTranslation}, locale})
    ...
    const blogsTitlesTranslated = await getTranslation({text: blogsTitles, locale})
    blogs?.data.map((blog, index) => blog.attributes.blog_title = blogsTitlesTranslated[index])
  }
  ...
  `{
    ...
    "itemListElement": [{
      ...
      "item": "https://stageproperties.com/blogs/page-${currPaginationPage}"
    }]
  }`

  const breadcrumbItems = [
    {
      title: <Link href='/blogs' className="breadcrumb">{t('blogs')}</Link>
    },
    {
      title: <p className="breadcrumb focus">{t('page')} {currPaginationPage}</p>
    }
  ]
  ...
  return (
    <>
      ...
      <div id="blogsListing">
        ...
        <h1 className={`mainHeading gradientText ${isRTL ? 'ar' : ''}`}>{pageHeadingTranslation}</h1>
        <p className={`description ${isRTL ? 'ar' : ''}`}>{briefIntroTranslation}</p>
