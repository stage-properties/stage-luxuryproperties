...
const findTitle = async (SecondaryPropertyData, locale) => {
  ...
  const category =
    propertyData?.attributes?.category?.data?.attributes?.category_name;
  const offeringType =
    propertyData?.attributes?.offering_type?.data?.attributes?.type;

  const emirate =
    propertyData?.attributes?.emirate?.data?.attributes?.emirate_name;
  const emirate_ar =
    propertyData?.attributes?.emirate?.data?.attributes?.localizations
      ?.data?.[0]?.attributes?.emirate_name;
  const _emirate = locale === "ar" ? emirate_ar ?? emirate : emirate;

  const propertyType =
    propertyData?.attributes?.property_type?.data?.attributes?.type;
  const _community =
    locale === "ar"
      ? propertyData?.attributes?.community?.data?.attributes?.localizations
          ?.data?.[0]?.attributes?.community_name ??
        propertyData?.attributes?.community?.data?.attributes?.community_name
      : propertyData?.attributes?.community?.data?.attributes?.community_name;
  ...
  const NumToBedroomsInAR = (number) => {
    ...
    if (number == 1)
      ...
      return locale === "ar" ? t("two-bedrooms") : `${number} ${t("bedrooms")}`;
    else if (number > 2)
      return `${
  ...
  const title =
    locale === "ar"
      ? arabicTitle + " " + property_ref_no.replaceAll("-", " - ") + " "
      : englishTitle + " " + property_ref_no.replaceAll("-", " - ");
...
export const generateMetadata = async ({ params }) => {
  ...
  const SecondaryPropertyData = await fetchSingleSecondaryProperty(
  ...
  const featuredImageURL =
    SecondaryPropertyData?.data?.attributes?.featured_image?.data?.attributes
      ?.url;
  const featuredImageAlternativeText =
    SecondaryPropertyData?.data?.attributes?.featured_image?.data?.attributes
      ?.alternativeText;

  const BASE_URL = `${origin}/`;

  const description = SecondaryPropertyData
    ? findDescription(SecondaryPropertyData)
    : "";
  //   const objToBeTranslated = {
  //     title: `${title} | Stage Properties`,
  //     description: `${SecondaryPropertyData ? findDescription(SecondaryPropertyData) : ''}`,
  //     featuredImageAlternativeText: featuredImageAlternativeText || `${title} | Stage Properties`
  // }

  //   const translations = await getTranslation({text: objToBeTranslated, locale})

  return {
    title: `${title} | ${t("Stage Properties")}`,
    description: description,
    alternates: {
      canonical: (BASE_URL + locale + "/rent/" + slug).replace("/en/", "/"),
      languages: {
        "en-gb": fullURL,
        en: fullURL,
        "x-default": fullURL,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: featuredImageAlternativeText || title + " | Stage Properties",
        },
      ],
      type: "website",
    },
