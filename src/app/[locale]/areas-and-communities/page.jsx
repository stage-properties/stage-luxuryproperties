...
export const generateMetadata = async ({params}) => {
  ...
  const canonicalURL = `https://stageproperties.com/${locale === 'ar' ? 'ar/' : ''}areas-and-communities`
...
const AreasAndCommunities = async ({params: { locale }}) => {
    ...
    const res_areaPage = await fetchAPI(`pageArea?locale=${locale}`, 'noCache')
    const res_communities = await fetchAPI(`communities/all?locale=${locale}`, 'noCache')
    ...
    const breadcrumbItems = [
        {
          title: <p className="breadcrumb focus">{t_areas_and_communities('areas_and_communities')}</p>
        }
    ]
    ...
    let res_communities_translated = res_communities?.data?.map((item) =>  item.attributes.description?.trim().length > 100 ? item.attributes.description?.substring(0, 100)?.trim() + '...' : item.attributes.description?.trim())
    const _description = res_communities_translated

    res_communities_translated = res_communities.data = res_communities?.data?.map((item, index) => {
      return {
        ...item,
        attributes: {
          ...item.attributes,
              ...
              <p className={`subtitle ${isRTL ? 'ar' : ''}`}>{description}</p>
                ...
                <div className="_container">
                {
                  res_communities_translated?.map((community) => {
                    ...
                    const alternativeText = community?.attributes?.community_image?.data?.attributes?.alternativeText
                ...
                <CTAContainer style={{marginBottom: '7rem', marginTop: '7rem', width: '99%', textAlign: 'center'}}/>
              <div className="contentContainer">
                <SubscribeNewsletter title={t_areas_and_communities("GET IN TOUCH WITH ONE OF OUR EXPERTS")} type='contact-form'/>
              </div>
          </div>
        </div>
    )
