...
// http://localhost:3000/buy/residential/properties-for-sale/in-damac-hills
...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const communitySlug = params["param-3"]
      ?.split("-")
      ?.slice(1)
      ?.join("-")
      .replaceAll("(", "")
    ...
    const emirate =
      community_res?.attributes?.emirate?.data?.attributes?.emirate_name;
    const community = community_res?.attributes?.community_name;

    const contructTitle = () => {
      if (
        params["param-2"] === "properties-for-sale" ||
        params["param-2"] === "properties-for-rent"
      ) {
        if (
          title &&
          (!params["param-3"] || params["param-3"].includes("page-"))
        ) {
          return title;
        } else {
          return `${t_common(propertyType.toLowerCase())} ${
            community ? ` ${t_common("in")} ${community}, ${emirate}` : "hello"
          }`;
        }
      } else {
        return `${t_common(propertyType.toLowerCase())} ${
    ...
    const constructDescription = () => {
      if (
        ...
        if (
          ...
          return `${t_common("Explore")} ${param1} ${t_common(propertyType)} ${
            community ? ` in ${community}, ${emirate}` : ""
          } ${t_common("with your trusted real estate partner")}`;
      } else
        return `${t_common("Explore")} ${param1} ${t_common(propertyType)} ${
          community ? ` in ${community}, ${emirate}` : ""
        } ${t_common("with your trusted real estate partner")}`;
