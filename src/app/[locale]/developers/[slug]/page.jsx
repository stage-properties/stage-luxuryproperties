...
export const generateMetadata = async ({ params }) => {
  ...
  const getDeveloperInfo = await fetchAPI(`developers/${slug}?locale=${locale}`, 'noCache')
  
  let canonicalURL = `https://stageproperties.com/${locale === 'ar' ? 'ar/' : ''}developers/${slug}`
  ...
  const developer_communities = getDeveloperInfo?.data[0]?.attributes?.communities
  const developer_name = getDeveloperInfo?.data[0]?.attributes?.developer_name
  const developer_image_alternativeText = getDeveloperInfo?.data[0]?.attributes?.developer_logo?.data?.attributes?.alternativeText

  let title = developer_communities?.data?.length ? `${developer_name} ${t('Off Plan Projects & Communities')}` : `${developer_name} ${t('Off Plan Projects')}`
...
const Developer = async ({params}) => {
  ...
  const { locale, slug } = params
  const t_developers = await getTranslations({locale, namespace: 'developers'});
  ...
  const getPropertiesByDeveloper = await fetchSearchResults(`slug=${developer_slug}&page=1&pageSize=9999&locale=${locale}`)
  ...
  const developer_description = getDeveloperInfo?.data[0]?.attributes?.developer_description || "";

  const developer_image = getDeveloperInfo?.data[0]?.attributes?.developer_logo?.data?.attributes?.url
  const developer_communities =  getDeveloperInfo?.data[0]?.attributes?.communities
  const developer_image_alternativeText = getDeveloperInfo?.data[0]?.attributes?.developer_logo?.data?.attributes?.alternativeText

  const breadcrumbItems = [
      {
          title: <Link href='/developers' className='breadcrumb'>{t_developers('developers')}</Link>
      }, 
      {
          title: <p className='breadcrumb focus'>{developer_name.toString().toUpperCase()}</p>
      }
  ]
  ...
  `{
    ...
    "itemListElement": [{
      ...
      "position": 3,
      "name": "Developer",
      "item": "https://stageproperties.com/developers/${params.developer}"
    }]
  }`
      ...
      <div className="wrapper developer">
          ...
          <div className="__top" dir='ltr'>
            <div className="isDesktop" dir={direction}>
              <h1 className="mainHeading __topProjects ___title">{developer_name}</h1>
            </div>
            <div className="isDesktop" dir={direction}>
              <DeveloperCardHeader 
            ...
            <div className="isMobile">
              <img className="mobileLogo" src={developer_image} alt={developer_image_alternativeText || developer_name} />
              <h1 className="__title">{developer_name}</h1>
              <p className={`__subtitle ${isRTL ? 'ar' : ''}`}>{developer_description}</p>
            </div>
            <div className="__seperator" />
            {getPropertiesByDeveloper?.data.length ? <h2 className="mainHeading __topProjects">{t_developers('Top Off Plan Projects by')} {developer_name}</h2> : null}
            {
              getPropertiesByDeveloper?.data.length >= 1 && getPropertiesByDeveloper?.data.length < 4 ? (
            ...
            <>
              <h2 className="mainHeading __topProjects">{t_developers('Top Communities Developed by')} {developer_name}</h2> 
              <div className="_container">
                {
                  developer_communities?.data?.map((community) => {
                    ...
                    const formatDescription = community?.attributes?.description?.toString()
                    const description = formatDescription?.trim().length > 100 ? formatDescription?.substring(0, 100)?.trim() + '...' : formatDescription?.trim()
                    return (
            ...
            {developer_communities?.data?.length ? (
              <Link href={'/areas-and-communities'} className="ctaBtnParent">
                <button className="globalBtn showMoreBtn ctaBtn">{t_developers('Check All Communities')}</button>     
              </Link>
            ) : null}
