...
export const generateMetadata = async ({ params }) => {
  ...
  const { locale } = params;
  const res_developerPage = await fetchAPI(
  ...
  const canonicalURL = `${origin}/${locale === "ar" ? "ar/" : ""}developers`;
...
const Developers = async ({ params }) => {
  ...
  return (
    <div className="wrapper developers" dir={direction}>
      ...
      <div className="top">
        <h1 className={`title ${isRTL ? "ar" : ""}`}>
          {t("real_estate_developers_in_dubai")}
        </h1>
        <p className={`subtitle ${isRTL ? "ar" : ""}`}>{subTitle}</p>
        <div className="isDesktop" style={{ display: "none" }}>
          <GradientLine width={"80%"} />
        </div>
        <div className="_container">
          {res_developerPage?.sorted_res_developers?.map((item, index) => {
            const alternativeText =
              item?.attributes?.developer_logo?.data?.attributes
                ?.alternativeText;
        ...
        <div className="contentContainer">
          <SubscribeNewsletter
            title={t("GET IN TOUCH WITH ONE OF OUR EXPERTS")}
            type="contact-form"
          />
        </div>
      </div>
